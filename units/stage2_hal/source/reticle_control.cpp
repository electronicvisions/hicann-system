...
std::multiset<uint> & ReticleControl::instantiated() {
	...
	if (!_i) _i = new std::multiset<uint>;
...
void ReticleControl::init(bool on_wafer=true) {
	...
	assert((model==jtag_wafer || model==jtag_wafer_fpga || model==jtag_eth_fpga_arq) && "The communication mode is not compatible with ReticleControl");
	...
	if (model==jtag_wafer_fpga){
		...
		jtag_p2f.reset(new S2C_JtagPhys2FpgaArq(*access.get(), jtag.get(), on_wafer,
		                                        jtag_port - 1700, kintex, tmp.to_string()));
		comm = jtag_p2f.get();
	} else {
	...
	//because JTAG address space is contiguous (indexing of hicann bitset) we use .count() (number of 1s)
	...
	kintex(_kintex)
{
	...
	//create fake reticle to comply with other functionality, set jtag port wether kintex (i.e. CubeSetup) is used
...
ReticleControl::ReticleControl(
	...
	kintex(_kintex)
{
	LOG4CXX_INFO(logger, "Creating ReticleControl instance for reticle IP/FPGA "
	                         << _ip << "/" << ((_ip.d - 1) % 32 + int(_ip.d / 32) * 12)
	                         << " (on wafer " << _ip.c << ") port " << port);
...
ReticleControl::~ReticleControl(){
	...
	if(!(instantiated().erase(s_number))) {
		LOG4CXX_ERROR(
		    logger, "There was a collision with reticle instances (" << x << "," << y
		                                                             << ") at destructor");
...
void ReticleControl::printInstantiated(){
	if (instantiated().empty()) cout << "No reticles have been instantiated so far" << endl;
	else {
...
bool ReticleControl::hicannInit(uint hicann_nr, bool silent, bool return_on_error){ //TODO: re-write for initAllConnections stuff...
	bool success=true;

	if (comm->Init(comm->dnc2jtag(hicann[hicann_nr]->addr()),/*silent*/ silent, /*force_highspeed_init*/ false, return_on_error) != Stage2Comm::ok) {
		LOG4CXX_WARN(
...
bool ReticleControl::hicannInit(std::bitset<8> hicann, bool silent, bool return_on_error){
	...
	if (comm->Init(hicann, /*silent*/ silent, /*force_highspeed_init*/ false, return_on_error) != Stage2Comm::ok) {
		LOG4CXX_WARN(
...
uint8_t ReticleControl::get_fpga_dnc_channel_id() const {
	...
	if (tmp > 3)
		throw logic_error("ReticleControl::get_fpga_dnc_channel_id(): jtag_port is not in range [1700, 1703]. Unable to detect FPGA-DNC channel");
	return tmp;
}
