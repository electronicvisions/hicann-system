...
private:
	struct hicann_config_t
	{
		...
		hicann_config_t(uint64_t set_data, unsigned int set_dnc, unsigned int set_hicann)
		    : dnc(set_dnc), hicann(set_hicann), data(set_data)
		{}
	};
	...
	// pair.second - 14bit pulse address, consisting of DNC ID, HICANN ID, channel ID and neuron
	// address
...
public:
	...
	/// return the currently sequence number that is used for the next frame.
	...
	bool sendSingleFrame(
	    ...
	    unsigned int user_data = 0, //!< lower 16 bit of the frame type header, if 0: insert number
	                                //!< of 32bit packets in payload
	    double const timeout =
	        500.0 //!< time [in ms] after which function returns if no acknowledge was received
	);
	...
	/// request HICANN data
	///
	bool readHICANN(uint64_t payload, double timeout = 100.0);
	...
	/// pulses must be sorted in time, times denote the release time at the FPGA in seconds (system
	/// time scale, not biological) and must be positive
	bool sendPulseList(
	    const std::vector<pulse_float_t>& pulses, double fpga_hicann_delay_in_s = 500.0e-9);
	...
	/// time denotes release time at the FPGA in seconds (system time scale, not biological) and
	/// must be positive the single configuration packet is sent directly to the FPGA in a separate
	/// UDP frame
	bool sendPlaybackConfig(
	    double fpga_time,
	    uint64_t payload,
	    unsigned int dnc = 0,
	    unsigned int hicann = 0,
	...
	bool setSystime(
	    ...
	    bool const expstart_mode,
	    double const timeout = 100.0);
	...
	/// send FPGA Layer2 routing data
	///
	bool sendFPGARouting(uint64_t payload, double timeout = 100.0);


	/// send DNC Layer2 routing data
	///
	bool sendDNCRouting(uint64_t payload, double timeout = 100.0);

	/// send DNC configuration
	///
	bool sendDNCConfiguration(uint64_t payload, double timeout = 100.0);

	/// send single HICANN config packet.
	bool sendSingleHICANNConfig(
		uint64_t payload, unsigned int dnc = 0, unsigned int hicann = 0, double timeout = 100.0);
	...
	uint64_t getReceivedHICANNConfig(
	    unsigned int dnc, unsigned int hicann, unsigned int tagid, double timeout = 20000.0);

	/// return number of HICANN config packets that are available in host receive buffer for the
	/// specified HICANN
	unsigned int getReceivedHICANNConfigCount(
	...
	/// set replace mode, where the timestamp fields of playback pulses are set to the system time
	/// counter at the time of their release
	bool setPlaybackSystimeReplacing(bool replace_enable, double timeout = 100.0);

	bool setHicannARQLoopback(bool loopback_enable, double timeout = 100.0);
...
private:
	...
	// maximum sequence number in hostal packet
	unsigned int const seq_number_max;

	unsigned int curr_seq_number;
	std::deque<unsigned int> received_seqnumbers;
	unsigned int curr_rec_seq_number;
	...
	std::vector<uint32_t> playback_entry_buffer;     /// buffer for playback entries. Is filled via
	                                             /// addPlaybackPulse() or addPlaybackConfig() and
	                                             /// cleared in flushPlaybackPulses()
	full_fpga_time_t last_playback_reltime; /// release time of last pulse in playback memory in
	                                        /// FPGA clk cycles(=8ns)
	...
	full_hicann_time_t last_traced_tstamp; /// holds the last traced time stamp (hicann clk cycles).
	                                       /// Use to determine counter overflows.
	unsigned int rec_trace_entries;        /// number of trace DDR2 entries received
	unsigned int pb_end_address_received;  /// last acknowledged end address of playback content, as
	                                       /// requested by playback end-of-data marker
