...
bool HostALController::sendSingleFrame( unsigned int frametype, unsigned int pllength, void *payload, unsigned int /*user_data*/, double /*timeout*/)
{
    if (pllength == 0)
    {
		LOG4CXX_WARN(logger, "HostALController::sendSingleFrame: trying to send frame without payload (type: 0x" << std::hex << frametype << std::dec << "). Ignoring.");
        return true;
    }
	...
	LOG4CXX_DEBUG(logger, "HostALController::sendSingleFrame: length(bytes): " << pllength << ", length (64bit words): " << curr_pck.len << ", type: " << std::hex << frametype << std::dec);
...
{
	if (sendSingleFrame(application_layer_packet_types::FPGACONFIG, sizeof(uint64_t), (void*)&payload, 0, 0) != true) {
...
#endif
	//FIXME remove all communication timeouts and let HostARQ handle timeouts with throws (ISSUE:2526)
	...
	while (difftime(now ,start) < cable_plug_timeout) {
		...
		if (curr_pck.pid != application_layer_packet_types::FPGACONFIG) {
			throw std::runtime_error("sendFPGAConfigPacket: unexpected response packet type " + std::to_string(curr_pck.pid));
		}
		if (curr_pck.len > 1) {
			throw std::runtime_error("sendFPGAConfigPacket: unexpected response packet length " + std::to_string(curr_pck.len));
		}
		uint64_t const response_payload = curr_pck.pdu[0];
		if (response_payload != payload) {
			throw std::runtime_error("sendFPGAConfigPacket: unexpected response packet payload " + std::to_string(response_payload) + " != " + std::to_string(payload));
		}
#ifndef NCSIM
		auto chrono_end = std::chrono::system_clock::now();
		auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(chrono_end - chrono_start);
		LOG4CXX_DEBUG(logger, "sendFPGAConfigPacket: received response after " + std::to_string(elapsed.count()) + "us");
#endif
		return true;
	}
	throw std::runtime_error("sendFPGAConfigPacket: did not receive response packet after " + std::to_string(cable_plug_timeout) + "s");
}
...
bool HostALController::waitForReceivedUDP(double timeout, double poll_interval)
{
	if (this->ethernet_if == NULL)
	{
		LOG4CXX_WARN(logger, "HostALController::initFPGAConnection: EthernetIFBase pointer not set.");
		return true;
	}
...
std::vector<pulse_float_t> HostALController::getFPGATracePulses(){ 
	...
	for ( size_t np = 0; np < num_pulses; ++np ) {
		...
		retval.push_back(pulse_float_t(raw_pulse.second, raw_pulse.first*this->dnc_clk_cycle_in_s));
	}
...
bool HostALController::sendFPGAPlayback(std::vector<uint32_t> &payload, double timeout)
{
	...
	{
		unsigned int framesize_32bit = (payload.size()-npos > max_frame) ? (max_frame) : (payload.size()-npos); 
		unsigned int userdata = framesize_32bit;
		retval &= this->sendSingleFrame(application_layer_packet_types::FPGAPLAYBACK, 4*framesize_32bit, p_ptr+npos, userdata, timeout);
	}
	
	return retval;
}

bool HostALController::sendPulseList(const std::vector<pulse_float_t> & pulses, double fpga_hicann_delay_in_s)
{
	for (unsigned int npulse=0;npulse<pulses.size();++npulse)
	{
		...
		uint16_t hicann_reltime = (uint16_t)( ( (pulse.time+fpga_hicann_delay_in_s) / dnc_clk_cycle_in_s ) + 0.5 );
...
bool HostALController::sendPlaybackConfig(double fpga_time, uint64_t payload, unsigned int dnc, unsigned int hicann, double timeout)
{
...
void HostALController::addPlaybackPulse(full_fpga_time_t fpga_time, full_hicann_time_t hicann_time, pulse_address_t id)
{
	LOG4CXX_DEBUG(logger, "HostALController::addPlaybackPulse: Processing new pulse with FPGA time " << std::dec << fpga_time
                         << ", HICANN time " << hicann_time << ", id " << id);

    unsigned int max_pulses_per_group = 184;
	 // too small distance -> continue current group
	if ( (fpga_time < last_playback_reltime + min_reltime_distance_clks) && (last_playback_reltime > 0)
        && (last_group_start_in_buffer >= 0) && ( ((this->playback_entry_buffer[last_group_start_in_buffer])>>18) < max_pulses_per_group ) )
	{
		unsigned int pulses_in_group = (this->playback_entry_buffer[last_group_start_in_buffer]) >> 18;
        
        if (pulses_in_group & 0x1) // odd number of pulses up to now -> start new 64bit entry
		{
			this->playback_entry_buffer.push_back( (((uint32_t)(id)&0x3fff)<<18) | ((hicann_time&0x7fff)<<1) );
			this->playback_entry_buffer.push_back( 0 );
		}
        else
        {
			this->playback_entry_buffer.back() = (((uint32_t)(id)&0x3fff)<<18) | ((hicann_time&0x7fff)<<1);
        }
        
        this->playback_entry_buffer[last_group_start_in_buffer] = ( this->playback_entry_buffer[last_group_start_in_buffer] & 0x3ffff ) | ((pulses_in_group+1) << 18);
        
		LOG4CXX_DEBUG(logger, "HostALController::addPlaybackPulse: Adding pulse to group at buffer size " << this->last_group_start_in_buffer
                             << ", fpga_time=" << fpga_time << ", last_playback_reltime=" << last_playback_reltime);
        
        last_playback_reltime += 1; // one clock needed for playing back one pulse
	}
	else // higher fpga_time distance -> start new group
    {
		// handle overflows -> assumes that pulse times are strictly sorted and 64bit integer value does not overflow
		if ( ( (fpga_time-last_playback_reltime) > 0x2000 ) && ( (last_playback_reltime>>14) < (fpga_time>>14) ) ) // have to generate overflow
		{
			full_fpga_time_t interval_diff = (fpga_time>>14) - (last_playback_reltime>>14);
		
			if ( (last_playback_reltime&0x3fff) > 0x3ffb) // take into account that playback FSM needs some clks to go into waiting state
			{
				interval_diff -= 1;
			}
			...
			{
				LOG4CXX_WARN(logger, "HostALController::addPlaybackPulse: inter-spike interval exceeds currently supported number of systime counter overflows: "
				                      << interval_diff << ">1023. Truncating interval.");
				interval_diff = 0x3fff;
			}

			LOG4CXX_DEBUG(logger, "HostALController::addPlaybackPulse: Adding overflow wait packet, delaying for " << interval_diff << " systime counter intervals");
        ...
        if (curr_fpga_time < last_playback_reltime + min_reltime_distance_clks)
        {
			...
			LOG4CXX_WARN(logger, "HostALController::addPlaybackPulse: Had to shift pulse by " << std::dec << (curr_fpga_time-fpga_time) << "FPGA clock cycles because of too high rate.");
        }
		...
		LOG4CXX_DEBUG(logger, "HostALController::addPlaybackPulse: Starting new pulse group at buffer size " << this->last_group_start_in_buffer);
...
void HostALController::addPlaybackConfig(full_fpga_time_t fpga_time, uint64_t payload, pulse_address_t hicann_id)
{
	unsigned int pck_count = 1;
	this->playback_entry_buffer.push_back( (pck_count<<18) | (1<<16) | ((fpga_time&0x3fff)<<1) );
	this->playback_entry_buffer.push_back( 0 );
...
bool HostALController::setSystime(const bool enable, const bool listen_global, const bool expstart_mode, const double /*timeout*/)
{
	uint64_t payload = (1<<22); // systime write enable bit

	if (enable) payload |= (1<<21);
	if (listen_global) payload |= (1<<20);
	if (expstart_mode) payload |= (1<<19);

	if (pulse_loopback_enable) payload |= (1<<24);
	if (pb_systime_replace_enable) payload |= (1<<23);

	return sendFPGAConfigPacket(payload);
}


bool HostALController::addPlaybackFPGAConfig(full_fpga_time_t fpga_time, bool pb_end_marker, bool stop_trace, bool start_trace_read)
{
	// handle overflows -> assumes that times are strictly sorted and 64bit integer value does not overflow
	if ( ( (fpga_time-last_playback_reltime) > 0x2000 ) && ( (last_playback_reltime>>14) < (fpga_time>>14) ) ) // have to generate overflow
	{
		full_fpga_time_t interval_diff = (fpga_time>>14) - (last_playback_reltime>>14);
		...
		if (interval_diff > 0x3fff)
		{
			LOG4CXX_WARN(logger, "HostALController::addPlaybackFPGAConfig: inter-spike interval exceeds currently supported number of systime counter overflows: "
								  << interval_diff << ">1023. Truncating interval.");
			interval_diff = 0x3fff;
		}

		LOG4CXX_DEBUG(logger, "HostALController::addPlaybackFPGAConfig: Adding overflow wait packet, delaying for " << interval_diff << " systime counter intervals");
	...
	if (stop_trace) config_low |= 1; // bit 0: stop trace
	if (start_trace_read) config_low |= (1<<1); // bit 1: start trace read

	uint32_t config_high = (1<<31); // bit 63(31): FPGA config
	if (pb_end_marker) config_high |= (1<<30); // bit 62(30): end marker
...
bool HostALController::sendSingleHICANNConfig(uint64_t payload, unsigned int dnc, unsigned int hicann, double timeout)
{
	unsigned int userdata = 2; // length=2 (2x32bit portions)
	unsigned int dest = (dnc&0x3)<<3 | (hicann&0x7);
	payload = (payload & 0x3ffffffffffffull) | ((uint64_t)(dest)<<59);// data is 49bit plus 1bit ARQ tag, destination in MSBs
	return this->sendSingleFrame(application_layer_packet_types::HICANNCONFIG, 8, &payload, userdata, timeout);
}
...
bool HostALController::sendHICANNConfigBulk(double timeout)
{
	...
	while (remaining_pck_count > 0)
	{
		...
		LOG4CXX_DEBUG(logger, "  -> Sending HICANN config frame with " << curr_pck_count  << " config packets:");
		for (unsigned int npck=0;npck<curr_pck_count;++npck)
		{
			...
			uint64_t curr_payload = (hiconf_bulk_buffer.at(pck_buffer_pos+npck).data & 0x3ffffffffffffull)
			                      | (static_cast<uint64_t>(dest) << 59);
			//printf("        packet % 2d: %016llX\n",npck,(unsigned long long)curr_payload);
			LOG4CXX_DEBUG(logger, "        packet " << npck<< ": " << std::hex << (unsigned long long)curr_payload // TODO fill payload with zeros
			                    << " (dnc " << dnc << ", hicann " << hic << ", " << (is_read ? "read" : "write") << ")");
			payload.push_back( curr_payload );// data is 49bit plus 1bit ARQ tag, destination in MSBs
		}

		uint64_t *p_ptr = &(payload.front());
		retval &= this->sendSingleFrame(application_layer_packet_types::HICANNCONFIG, 8*curr_pck_count, p_ptr, userdata, timeout);
...
uint64_t HostALController::getReceivedHICANNConfig(unsigned int dnc, unsigned int hicann,
                                                   unsigned int tagid, double const timeout) {
	int ret;

	if ( (dnc>3) || (hicann>7) )
	{
		LOG4CXX_ERROR(logger, "HostALController::getReceivedHICANNConfig: Invalid DNC or HICANN specified: DNC " << dnc << ", HICANN " << hicann <<". Ignoring request.");
		#ifndef NCSIM
		LOG4CXX_FATAL(
	...
	std::tr1::array<size_t, 9> const sleep_intervals = {5, 10, 100, 500, 1000, 5000, 10000, 100000, 500000};
	...
	for (size_t sleep_interval_idx = 0; hiconf_receive_buffer.at(curr_id).size() == 0;) {
		if (! arq_ptr->received_packet_available()) {
			...
			if (ret > 0) {
				LOG4CXX_ERROR(logger, "HostALController::getReceivedHICANNConfig: "
		...
		if (accumulated_sleep > timeout*1000) {
			LOG4CXX_ERROR(logger, "HostALController::getReceivedHICANNConfig: No config packet received for DNC " << dnc << ", HICANN "<< hicann << " after waiting " << accumulated_sleep << ". Return 0.");
			#ifndef NCSIM
			LOG4CXX_FATAL(
			    logger, "HostALController::getReceivedHICANNConfig() ["
			                << target_ip << ":" << target_port
...
unsigned int HostALController::getReceivedHICANNConfigCount(unsigned int dnc, unsigned int hicann, unsigned int tagid)
{
	if ( (dnc>3) || (hicann>7) )
	{
		LOG4CXX_ERROR(logger, "HostALController::getReceivedHICANNConfig: Invalid DNC or HICANN specified: DNC " << dnc << ", HICANN " << hicann <<". Ignoring request.");
...
bool HostALController::startTraceRead(double /*timeout*/)
{
	...
	LOG4CXX_DEBUG(logger, "setPlaybackSystimeReplacing: content=" << std::hex << payload << std::dec);
...
void HostALController::fill_receive_buffer(bool /*with_jtag*/)
{
	...
	while ( arq_ptr->receive(curr_pck) ) {
		...
		LOG4CXX_DEBUG(logger, "fill_receive_buffer: received hostARQ packet with " << curr_pck.len << " entries.");
        ...
        if (curr_pck.len)
        {
			switch (frame_type)
			{
				...
				{
					LOG4CXX_DEBUG(logger, "HostALController: received frame " << pck_count-1 << " with ARQ data (number of 64-bit entries: " << curr_pck.len << ").");

					for (unsigned int ndata = 0; ndata < curr_pck.len; ++ndata) // we access 64-bit entries!
					{
						LOG4CXX_DEBUG(logger, "HostAL Raw packet: " << std::setfill('0') << std::setw(8) << std::hex << curr_pck.pdu[ndata]);
						...
						LOG4CXX_DEBUG(logger, "  -> Received config packet from DNC " << cfgdata.bitfield.dnc
							<< ", HICANN " << 7-cfgdata.bitfield.hicann
							<< ", Tag: " << cfgdata.bitfield.tag
							<< "): " << std::setfill('0') << std::setw(16) << std::hex << curr_pck.pdu[ndata]);
						...
						hiconf_receive_buffer.at(curr_id.raw).push_back( cfgdata.bitfield.hicann_data );
					}
					break;
				}
				...
				case application_layer_packet_types::FPGATRACE : 
				{
					...
					LOG4CXX_DEBUG(logger, "HostALController: received frame with trace data (number of entries: " << curr_pck.len << ").");

					LOG4CXX_DEBUG(logger, "  -> pulses in buffer so far: " << received_pulses_count);

					rec_trace_entries += curr_pck.len;

					LOG4CXX_DEBUG(logger, "Number of trace DDR2 entries received (including current frame): " << rec_trace_entries);
					...
					for (unsigned int ndata = 0; ndata < curr_pck.len; ++ndata) {
						...
						for (size_t ii = 0; ii < 2; ii++) {
                            ...
                            {
                                if ( (ii==0) || (timepulse.single_pulse.ov_pulse_ignore) ) // use second round of loop if pulse is contained in packet
                                    continue;
							...
							LOG4CXX_DEBUG(logger, "HostALController info: received pulse packet " << received_pulses_count << " from trace memory: "
								<< "id=0x" << std::hex << curr_id << ", tstamp=0x"
								<< std::hex << curr_tstamp << ", tstamp msb: " << tstamp_msb << " fpga msb: " << fpga_msb);

							last_traced_tstamp = curr_tstamp;

							full_fpga_time_t curr_inttime = (uint64_t)(curr_tstamp) + trace_overflow_count*MAX_TIMESTAMP_CNT; // FIXME
                            
                            // for pulse contained in overflow packet: revert increment for overflow counter
                            bool ignore_early_pulse = false;
                            if ((!fpga_msb) && tstamp_msb) // detect special case that HICANN timestamp was registered before overflow, but pulse arrives in FPGA after overflow and an overflow packet was generated
							...
							LOG4CXX_DEBUG(logger, "  Extraced time: " << curr_time << ", id: " << std::dec << curr_id << ", overflow count: " << trace_overflow_count);

							// Old bug where trace memory potentially stored pulse twice while sending overflow packet, should not occure anymore
							// TODO 2016-04-27: remove check when its absolutly sure that the bug is fixed
							if (recpulse_buffer.size() && (recpulse_buffer.back().second==curr_id)
								&& ((recpulse_buffer.back().first==curr_inttime)||(recpulse_buffer.back().first+MAX_TIMESTAMP_CNT==curr_inttime)) )
							{
								LOG4CXX_WARN_BACKTRACE(logger, "HostALController::fill_receive_buffer(): Received pulse twice ISSUE 2022");
								LOG4CXX_WARN(
									logger, "in received hostARQ packet with frame type "
									<< curr_pck.pid << " and " << curr_pck.len << " entries.");
								continue;
							}
						...
						{
							LOG4CXX_DEBUG(logger, "Overflow packet with value 0x" << std::hex << timepulse.single_pulse.overflow << " received");
				...
				case application_layer_packet_types::FPGAPLAYBACK:
				{
					...
					LOG4CXX_DEBUG(logger, "HostALController: received end-of-data message from playback with address " << std::dec << this->pb_end_address_received);
					break;
				}
				...
				default:
				{
					LOG4CXX_ERROR(logger, "HostALController: unknown frame type of received data: " << std::hex << frame_type << ". Ignoring data.");
					break;
				}
			}
		}
