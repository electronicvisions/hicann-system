...
class TmJSStdpCtrl : public Testmode /*, public RamTestIntfUint*/ {
protected:
	virtual string ClassName() {stringstream ss; ss << "tmjs_stdpctrl"; return ss.str();}
public:
...
//		ll->SetDebugLevel(3); // no ARQ deep debugging
		...
		rc = new CtrlModule(hc, TAG, ocp_base, 0x7fff); //StdpCtrl blocks use xx address bits + SRAM control bit + one additional control bit??
	...
	// ----------------------------------------------------
		...
		for(uint r=startrow;r<maxrows;r+=2){
			if( (r&0xff)>223){r+=0x100;r&=0x300;}
			write(sram+r,(tdata[r]=rand()&0xff)<<8);
		...
		for(uint r=startrow;r<maxrows;r+=2){
			...
			if(!(r%8))cout<<endl;
			cout <<read_ad(sram+r);
			if(((rcvdata>>8)&0xff) !=tdata[r])
				{error++;cout<<endl<<"Error at address: "<<rcvaddr<<" read: "<<bino(rcvdata>>8,8)<<" written: "<<bino(tdata[r],8)<<endl;
				 read_ad(sram+r);//read a second time
				 if(((rcvdata>>8)&0xff)!=tdata[r])
					{cout<<endl<<"Second read: "<<rcvaddr<<" read: "<<bino(rcvdata>>8,8)<<" written: "<<bino(tdata[r],8)<<endl;}
				}
		...
		startrow=0;
		maxrows=224;
		for(uint r=startrow;r<maxrows;r++){
...
class LteeTmJSStdpCtrl : public Listee<Testmode>{
public:
	LteeTmJSStdpCtrl() : Listee<Testmode>(string("tmjs_stdpctrl"), string("test stdpctrl synapse access etc")){};
	Testmode * create(){return new TmJSStdpCtrl();};
};

LteeTmJSStdpCtrl ListeeTmJSStdpCtrl;
