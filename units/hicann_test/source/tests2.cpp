...
* 04.08.08: (AG): completely moved multi-threading parts to Stage2Comm child (e.g. S2C_TcpIp->Stage2Comm)
*/
...
template<> Listee<Testmode> * Listee<Testmode>::first=NULL;//this allows access for all Listees
...
int main(int argc, char * argv[]){
	...
	enum options {none,opthelp,opttime,sr,js,optmode,listmodes,config,logger,tlabel,jthcnum,jthcaddr,dcnr,dcnl,stdiokeys,ipset,jpset,onret,bulksize,shared_memory_name,hicann_bitmask} actopt=none;  ///< check for command line options
	...
	istringstream keys(""); string key = ""; vector<string> keysequence; keysequence.clear();
	...
	bool on_reticle=false; ///< determines wether running on wafer reticle (affects dnc addressing)
	uint reticle_nr; ///< reticle nr according to dan/maurice's coordinate system
	uint dnc_hc_nr;
	uint mybulksize = 63; ///< default bulk size
	uint dnc_chnl[8] = {0, 1, 2, 3, 4, 5, 6, 7}; ///< assign dnc channels to logically constructed HICANNs
	FPGAConnectionId::IPv4::bytes_type fpga_ip = {{192, 168, 2, 1}}; ///< define default FPGA IP address
	uint jtag_port = 1700; ///< define default port for JTAG-over-Ethernet communication
	printf("Default FPGA IP address: %d.%d.%d.%d\n",fpga_ip[0],fpga_ip[1],fpga_ip[2],fpga_ip[3]);
...
//		if((actopt==none) && !strcmp(argv[j],"-d")){dbg.setLevel(Debug::debug);continue;}
		if((actopt==none) && !strcmp(argv[j],"-bjw")){commodel=jtag_wafer;actopt=none;continue;}
		if((actopt==none) && !strcmp(argv[j],"-bjw2f")){commodel=jtag_wafer_fpga;actopt=none;continue;}
		if((actopt==none) && !strcmp(argv[j],"-s")){actopt=opttime;continue;}
		if((actopt==none) && !strcmp(argv[j],"-?")){actopt=opthelp;}
		if((actopt==none) && !strcmp(argv[j],"-h")){actopt=opthelp;}
		if((actopt==none) && !strcmp(argv[j],"-sr")){actopt=sr;continue;}
		if((actopt==none) && !strcmp(argv[j],"-js")){actopt=js;continue;}
		if((actopt==none) && !strcmp(argv[j],"-bj")){commodel=jtag;actopt=jthcnum;continue;}
		if((actopt==none) && !strcmp(argv[j],"-bj2f")){commodel=jtag_2fpga;actopt=jthcnum;continue;}
		if((actopt==none) && !strcmp(argv[j],"-bje2f")){commodel=jtag_eth_fpga;actopt=jthcnum;continue;}
		if((actopt==none) && !strcmp(argv[j],"-bje2fa")){commodel=jtag_eth_fpga_arq;actopt=jthcnum;continue;}
		if((actopt==none) && !strcmp(argv[j],"-bje")){commodel=jtag_eth;actopt=jthcnum;continue;}
		if((actopt==none) && !strcmp(argv[j],"-bja")){commodel=jtag_full;actopt=jthcnum;continue;}
		if((actopt==none) && !strcmp(argv[j],"-dryrun")){commodel=dryrun;actopt=jthcnum;continue;}
		if((actopt==none) && !strcmp(argv[j],"-bjac")){commodel=jtag_multi;actopt=jthcnum;continue;}
		if((actopt==none) && !strcmp(argv[j],"-m")){actopt=optmode;continue;}
		if((actopt==none) && !strcmp(argv[j],"-c")){actopt=config;continue;}
		if((actopt==none) && !strcmp(argv[j],"-log")){actopt=logger;continue;}
		if((actopt==none) && !strcmp(argv[j],"-label")){actopt=tlabel;continue;}
		if((actopt==none) && !strcmp(argv[j],"-dc")){actopt=dcnr;continue;}
		if((actopt==none) && !strcmp(argv[j],"-kill")){kill_on_recvfromerror=true;continue;}
		if((actopt==none) && !strcmp(argv[j],"-bulksize")){actopt=bulksize;continue;}
		if((actopt==none) && !strcmp(argv[j],"-k")){actopt=stdiokeys;continue;}
		if((actopt==none) && !strcmp(argv[j],"-ip")){actopt=ipset;continue;}
		if((actopt==none) && !strcmp(argv[j],"-jp")){actopt=jpset;continue;}
		if((actopt==none) && !strcmp(argv[j],"-k7")){actopt=none;k7_setup=true;continue;}
		if((actopt==none) && !strcmp(argv[j],"-r")){actopt=onret;on_reticle=true;continue;}
		if((actopt==none) && !strcmp(argv[j],"-shm_name")){actopt=shared_memory_name;continue;}
		if((actopt==none) && !strcmp(argv[j],"-bitmask")){actopt=hicann_bitmask;continue;}
		if((actopt==none) && !strcmp(argv[j],"--")){break;} // end of known options
		
		if(actopt==opttime){sscanf(argv[j],"%ud",&starttime);actopt=none;continue;}
		if(actopt==optmode){modes.push_back(string(argv[j]));actopt=none;continue;}//store testmode
		if(actopt==sr){srand(atoi(argv[j]));randomseed=atoi(argv[j]);actopt=none;continue;}
		if(actopt==js){sscanf(argv[j],"%ud",&jtag_speed);actopt=none;continue;}
		if(actopt==config){confFileName = argv[j];actopt=none;continue;}
		if(actopt==logger){sscanf(argv[j],"%ud",&log_level);actopt=none;continue;}
		if(actopt==tlabel){label=argv[j];actopt=none;continue;}
		if(actopt==jthcnum){sscanf(argv[j],"%ud",&num_jtag_hicanns);actopt=jthcaddr;continue;}
		if(actopt==jthcaddr){sscanf(argv[j],"%ud",&tar_jtag_hicann);actopt=none;continue;}
		if(actopt==dcnr){sscanf(argv[j],"%ud",&dnc_hc_nr);actopt=dcnl;continue;}
		if(actopt==dcnl){sscanf(argv[j],"%ud",&dnc_chnl[dnc_hc_nr]);actopt=none;continue;}
		if(actopt==bulksize){sscanf(argv[j],"%ud",&mybulksize);actopt=none;continue;}
		if(actopt==onret){sscanf(argv[j],"%ud",&reticle_nr);actopt=none;continue;}
		...
		if(actopt==stdiokeys){
			keys.str(argv[j]);
			while(getline(keys,key,','))keysequence.push_back(key);
			actopt=none;
			continue;
		}
		if(actopt==ipset){
			sscanf(argv[j],"%hhd.%hhd.%hhd.%hhd",&fpga_ip[0], &fpga_ip[1], &fpga_ip[2], &fpga_ip[3]);
			if (shm_name.empty()) {
				stringstream shm_string;
				shm_string << argv[j];
				shm_name = shm_string.str();
			}
			printf("Changing IP address to %d.%d.%d.%d\n", fpga_ip[0], fpga_ip[1], fpga_ip[2], fpga_ip[3]);
			actopt=none;continue;
		}
		if(actopt==jpset){
		...
		if((actopt==none) && !strcmp(argv[j],"-l")){
			...
			while(t!=NULL){cout<<"\""<<t->name()<<"\" \""<<t->comment()<<"\""<<endl;t=t->getNext();}
			exit(EXIT_SUCCESS);
		}
		if(actopt==none)cout<<"Illegal options."<<endl;
		cout<<usagestr<<endl;
		if(actopt!=opthelp)exit(EXIT_FAILURE);
		else exit(EXIT_SUCCESS);
	}
	if(actopt!=none){cout<<"Missing argument (use -? for more information)."<<endl;exit(EXIT_FAILURE);}

	// parse rest of options
	bpo::options_description desc;
	bpo::parsed_options opts = bpo::command_line_parser(argc-j, argv+j).options(desc).allow_unregistered().run();
	...
	vector<Stage2Ctrl *> chips;     ///< all Stage2Ctrl(FpgaCtrl, DncCtrl, HicannCtrl) instances in the system 
	...
	switch(commodel){
		...
		case jtag_eth_fpga:{
			if (active_hicanns.any())
				...
				jtf   = new myjtag_full(true, !k7_setup, num_jtag_hicanns, tar_jtag_hicann, k7_setup);
			jtag_p2f = new S2C_JtagPhys2Fpga(access, jtf, on_reticle, k7_setup); // ECM: add connection parameters to comm class? jtf is ugly parameter? check with andi
			comm     = jtag_p2f;
			log(Logger::INFO)<< "JTAG chain communication model selected" << flush;
			log(Logger::INFO)<< "Using ETHERNET device..." << flush;
			log(Logger::INFO)<< "Initialized application layer" << flush;
		...
		case jtag_eth_fpga_arq:{
			...
			if (active_hicanns.any())
				...
				jtf   = new myjtag_full(true, !k7_setup, num_jtag_hicanns, tar_jtag_hicann, k7_setup);
			jtag_p2fa = new S2C_JtagPhys2FpgaArq(connid, jtf, on_reticle, jtag_port-1700, k7_setup, shm_name);
		...
		case jtag_wafer_fpga:{ //testing communication, will be deleted and reinstated in ReticleControl later
			log(Logger::INFO)<< "Wafer with JTAG to FPGA only communication model selected" << flush;
		}break;
	...
	if(commodel!=jtag_wafer && commodel!=jtag_wafer_fpga){
		...
		for(int c=0;c<FPGA_COUNT;c++) {FPGAControl* tmp = new FPGAControl(NULL,0); chips.push_back(tmp);}
		for(int c=0;c<DNC_COUNT;c++) {DNCControl* tmp = new DNCControl(NULL,0); chips.push_back(tmp);}
		for(uint c=0;c<HICANN_COUNT;c++) {HicannCtrl* tmp = new HicannCtrl(NULL,0); chips.push_back(tmp);}
	}
