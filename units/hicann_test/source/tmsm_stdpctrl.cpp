...
class TmSMStdpCtrl : public Testmode /*, public RamTestIntfUint*/ {
protected:
	virtual string ClassName() {stringstream ss; ss << "tmsm_stdpctrl"; return ss.str();}
public:
...
//		ll->SetDebugLevel(3); // no ARQ deep debugging
		...
		rc = new CtrlModule(hc->tags[TAG], ocp_base, 0x7fff); //StdpCtrl blocks use xx address bits + SRAM control bit + one additional control bit??
...
class LteeTmSMStdpCtrl : public Listee<Testmode>{
public:
	LteeTmSMStdpCtrl() : Listee<Testmode>(string("tmsm_stdpctrl"), string("test stdpctrl synapse access etc")){};
	Testmode * create(){return new TmSMStdpCtrl();};
};

LteeTmSMStdpCtrl ListeeTmSMStdpCtrl;
