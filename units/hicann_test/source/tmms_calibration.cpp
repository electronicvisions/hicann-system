...
class TMCalibration : public Testmode, public RamTestIntfUint {
protected:
	virtual string ClassName() {stringstream ss; ss << "tmsm_calibration"; return ss.str();}
public:
	...
	// Programm floating gates
		...
		do{
			...
			cout << "e: erase FG" << endl;
			cout << "c: config DNC and SPL1" << endl;
	...
	// Read data
		...
		tend = time(0);
		cout << "It took " << (tend-tstart) << " s to read spikes" << endl;
	} break;
	
	case 'e':{
	for(int i=0; i<24; i+=1){
			for(cntlr=0; cntlr<4; cntlr+=1){
				while(fgc[cntlr]->isBusy());

				log(Logger::DEBUG2)<<"programing row " << i << endl;
				 
				// start state machine
				fgc[cntlr]->program_cells(i,0,0);
			}
		}	
		for(cntlr=0; cntlr<4;cntlr+=1){

			fgc[cntlr]->readout_cell(1,0);
...
public:
	LteeTMCalibration() : Listee<Testmode>(string("tmms_calibration"), string("Calibration testmode")){};
