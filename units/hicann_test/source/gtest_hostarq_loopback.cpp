...
#define EXPECTED_TIME (sctrltp::ParametersFcpBss1::MAX_PDUWORDS * NUM_PACKETS * 8) / (MIN_RATE_IN_MBS*1024*1024)
...
void* sending_loop(void* p) {
	...
	for (size_t i = 0; i < NUM_PACKETS; i++) {
		for (size_t j = 0; j < sctrltp::ParametersFcpBss1::MAX_PDUWORDS; j++) {
			tmp.pdu[j] = globaldata[i*sctrltp::ParametersFcpBss1::MAX_PDUWORDS+j];
		}
		arq.send(tmp, sctrltp::ARQStream<sctrltp::ParametersFcpBss1>::NOTHING);
	}
	arq.flush();
	while (! arq.all_packets_sent())
		usleep(1000); // 1ms
	timediff = mytime() - timediff;
	size_t bytes_sent = NUM_PACKETS*sctrltp::ParametersFcpBss1::MAX_PDUWORDS*8;
...
void* receiving_loop(void* p) {
	...
	double timediff = mytime();
	for (size_t i = 0; i < NUM_PACKETS; i++) {
		arq.receive(tmp, sctrltp::ARQStream<sctrltp::ParametersFcpBss1>::NOTHING);
		for (size_t j = 0; j < sctrltp::ParametersFcpBss1::MAX_PDUWORDS; j++) {
			if (tmp.pdu[j] != globaldata[i*sctrltp::ParametersFcpBss1::MAX_PDUWORDS+j]) {
				errors++;
			}
		}
	}
	...
	std::cout << "receiver done after " << timediff << "s, " << errors << " errors out of " << NUM_PACKETS*sctrltp::ParametersFcpBss1::MAX_PDUWORDS << std::endl;
	pthread_exit(NULL);
}


void do_loopback_tests(std::vector<sctrltp::ARQStream<sctrltp::ParametersFcpBss1>*> arq) {
	// fill global data
	double timediff = mytime();
	globaldata = new uint64_t[NUM_PACKETS*sctrltp::ParametersFcpBss1::MAX_PDUWORDS];
	srand(1234);
	for (size_t i = 0; i < NUM_PACKETS; i++)
		globaldata[i] = rand();
	timediff = mytime() - timediff;
...
TEST(HostARQ, Loopback) {
	...
	std::vector<sctrltp::ARQStream<sctrltp::ParametersFcpBss1>*> arq;
	for (size_t i = 0; i < shm_name.size(); i++)
		arq.push_back(new sctrltp::ARQStream<sctrltp::ParametersFcpBss1>(shm_name[i]));
