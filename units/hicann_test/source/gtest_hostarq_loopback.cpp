...
#ifndef MAX_PDUWORDS
#error MAX_PDUWORDS not defined
#endif

#define NUM_PACKETS 1000000
#define MIN_RATE_IN_MBS 10
#define EXPECTED_TIME (MAX_PDUWORDS * NUM_PACKETS * 8) / (MIN_RATE_IN_MBS*1024*1024)
...
void* sending_loop(void* p) {
	...
	std::cout << "sending " << (8.0 * MAX_PDUWORDS * NUM_PACKETS / 1024 / 1024) << "MB" << std::endl;

	sctrltp::packet tmp;
	tmp.pid = HOSTARQ_LOOPBACK_TYPE;
	tmp.len = MAX_PDUWORDS;

	double timediff = mytime();
	for (size_t i = 0; i < NUM_PACKETS; i++) {
		for (size_t j = 0; j < MAX_PDUWORDS; j++) {
			tmp.pdu[j] = globaldata[i*MAX_PDUWORDS+j];
		}
	...
	size_t bytes_sent = NUM_PACKETS*MAX_PDUWORDS*8;
...
void* receiving_loop(void* p) {
	...
	for (size_t i = 0; i < NUM_PACKETS; i++) {
		...
		for (size_t j = 0; j < MAX_PDUWORDS; j++) {
			if (tmp.pdu[j] != globaldata[i*MAX_PDUWORDS+j]) {
				errors++;
			}
		}
	}
	...
	std::cout << "receiver done after " << timediff << "s, " << errors << " errors out of " << NUM_PACKETS*MAX_PDUWORDS << std::endl;
	pthread_exit(NULL);
}
...
void do_loopback_tests(std::vector<sctrltp::ARQStream*> arq) {
	// fill global data
	double timediff = mytime();
	globaldata = new uint64_t[NUM_PACKETS*MAX_PDUWORDS];
	srand(1234);
	for (size_t i = 0; i < NUM_PACKETS; i++)
		globaldata[i] = rand();
	timediff = mytime() - timediff;
