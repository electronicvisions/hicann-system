...
class TmJSRepeater : public Testmode /*, public RamTestIntfUint*/ {
protected:
...
//				rcw(repadr(1),rc_recen|rc_touten); 
		...
		//start dll operation and tdi and tdo
			...
			case 'n': { //activate sending repeaters and background generators to output 1 MHz events, for neuron stimulation
				...
				uint fstim;
				cin>>fstim;
				uint random = 0xffff;//random mask for bg
...
class LteeTmJSRepeater : public Listee<Testmode>{
public:
	LteeTmJSRepeater() : Listee<Testmode>(string("tmjs_repeater"), string("continous r/w/comp to distant mems e.g. switch matrices")){};
	Testmode * create(){return new TmJSRepeater();};
};

LteeTmJSRepeater ListeeTmJSRepeater;
