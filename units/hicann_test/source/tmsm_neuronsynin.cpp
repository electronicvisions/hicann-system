...
class TMSMNeuronSynin : public Testmode, public RamTestIntfUint {
protected:
	virtual string ClassName() {stringstream ss; ss << "tmsm_neuronsynin"; return ss.str();}
public:
...
//		ll->SetDebugLevel(3); // no ARQ deep debugging
		...
		log(Logger::DEBUG1)<<"tmsm_neuronsynin::Setting fg control parameters";
		uint fg_num;
		...
		log(Logger::INFO)<<"tmsm_neuronsynin::writing currents ...";
		...
		log(Logger::INFO)<<"tmsm_neuronsynin::writing voltages ...";
...
//paper			fc[fg_num]->initSquare(0,420);
...
/*	
		...
		log(Logger::DEBUG0)<< "tmsm_neuronsynin::ok" << endl;
...
class LteeTMNeuronSynin : public Listee<Testmode>{
public:
	LteeTMNeuronSynin() : Listee<Testmode>(string("tmsm_neuronsynin"), string("used for test of synaptic input")){};
	Testmode * create(){return new TMSMNeuronSynin();};
};

LteeTMNeuronSynin ListeeTMNeuronSynin;
