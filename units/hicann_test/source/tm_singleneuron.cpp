...
class TMSingleNeuron : public Testmode, public RamTestIntfUint {
protected:
	virtual string ClassName() {stringstream ss; ss << "tm_singleneuron"; return ss.str();}
public:
...
//		ll->SetDebugLevel(3); // no ARQ deep debugging
		...
		log(Logger::INFO)<<"tm_singleNeuron: starting...." << endl;
		...
		log(Logger::DEBUG1)<<"tm_singleneuron::Setting fg control parameters";
		uint fg_num;
		...
		log(Logger::INFO)<<"tm_singleneuron::writing currents ...";
		...
		log(Logger::INFO)<<"tm_singleneuron::writing voltages ...";
...
/*	
		...
		log(Logger::DEBUG0)<< "tm_singleneuron::ok" << endl;
...
class LteeTMSingleNeuron : public Listee<Testmode>{
public:
	LteeTMSingleNeuron() : Listee<Testmode>(string("tm_singleneuron"), string("look at a single neuron")){};
	Testmode * create(){return new TMSingleNeuron();};
};

LteeTMSingleNeuron ListeeTMSingleNeuron;
